        ...
        if (in_array($ext, $allowed)) {
            if ($_FILES["foto"]["size"] <= 2000000) {
                if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_dir . $new_name)) {
                    $gambar_value = "/lab11_php_oop/" . $relative_path . $new_name;
                } else {
    ...
    if (empty($message)) {
        ...
        try {
            if ($db->runQuery($sql, $params)) {
                echo "<script>alert('Artikel berhasil dipublikasikan!'); window.location.href='/lab11_php_oop/index.php/artikel/index';</script>";
                exit;
            }
        } catch (Exception $e) {
