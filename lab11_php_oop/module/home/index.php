 ...
 * Memastikan sesi login aktif sebelum mengakses dashboard.
 */
...
try {
    ...
    // Ambil Data User Aktif (Untuk Foto Profil Asli)
    $stmt_user = $db->runQuery("SELECT * FROM user WHERE username = :u", [':u' => $username_session]);
    $user_data = $stmt_user->fetch(PDO::FETCH_ASSOC);

    // Cek kolom is_active secara dinamis
    $check_col = $db->runQuery("SHOW COLUMNS FROM user LIKE 'is_active'")->rowCount();
    if ($check_col > 0) {
        $user_aktif = $db->runQuery("SELECT id FROM user WHERE is_active = 1")->rowCount();
    } else {
        $user_aktif = $total_user;
    }

    // Ambil 3 Artikel Terbaru untuk Feed
...
// Logika Tampilan Foto (Gunakan Avatar otomatis jika gambar kosong)
...
<div class="fade-in px-3 mt-3 mb-5">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden position-relative">
        <div class="card-body p-4 p-md-5 text-white" style="background: linear-gradient(120deg, #4e73df, #224abe);">
            <i class="fas fa-rocket position-absolute text-white opacity-10" style="font-size: 10rem; right: -20px; bottom: -20px;"></i>
        ...
        <div class="col-lg-7 mb-4">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body pt-0">
                    <div class="row g-2">
                        <div class="col-6">
                            <a href="/lab11_php_oop/index.php/artikel/tambah" class="btn btn-outline-primary w-100 p-3 rounded-3 text-start transition-up">
                                <i class="fas fa-pen-nib mb-2 d-block fa-lg"></i> Tulis Artikel
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="/lab11_php_oop/index.php/user/profile" class="btn btn-outline-dark w-100 p-3 rounded-3 text-start transition-up">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="p-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="bg-success bg-opacity-10 text-success rounded-circle p-2 me-3"><i class="fas fa-check-circle"></i></div>
                            <div>
                                <p class="small mb-0 fw-bold text-dark">Sesi Terverifikasi</p>
                                <small class="text-muted">Akses administrator aktif.</small>
                            </div>
                        </div>
                        <div class="d-flex align-items-center">
                            <div class="bg-primary bg-opacity-10 text-primary rounded-circle p-2 me-3"><i class="fas fa-fingerprint"></i></div>
                            <div>
                                <p class="small mb-0 fw-bold text-dark">Keamanan Hash</p>
                                <small class="text-muted">Password anda terlindungi.</small>
                            </div>
                        </div>
                    </div>
