...
if (isset($_POST['hapus_foto'])) {
    ...
    if (!empty($cek_foto['foto'])) {
        $file_path = __DIR__ . "/../../" . str_replace("/lab11_php_oop/", "", $cek_foto['foto']);
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (in_array($ext, $allowed) && $_FILES["foto"]["size"] <= 2000000) {
            ...
            if (!empty($old_data['foto'])) {
                $old_path = __DIR__ . "/../../" . str_replace("/lab11_php_oop/", "", $old_data['foto']);
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_dir . $new_name)) {
                $foto_value = "/lab11_php_oop/assets/img/user/" . $new_name;
            }
        } else {
