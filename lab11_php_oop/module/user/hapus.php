...
try {
    ...
    if ($user_data) {
        ...
        if (!empty($user_data['foto'])) {
            ...
            $relative_path = ltrim(str_replace("/lab11_php_oop/", "", $user_data['foto']), "/");
            $full_file_path = $_SERVER['DOCUMENT_ROOT'] . "/lab11_php_oop/" . $relative_path;
        ...
        if ($db->runQuery($sql_delete, [':id' => $id])) {
            echo "<script>alert('Akun pengguna berhasil dihapus secara permanen!'); window.location.href='/lab11_php_oop/index.php/user/index';</script>";
            exit;
        }
    } else {
        header("Location: /lab11_php_oop/index.php/user/index");
